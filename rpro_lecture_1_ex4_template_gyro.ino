...
int32_t getTurnAngleInDegrees(){
  turnSensorUpdate();
  return((((int32_t)turnAngle >> 16) * 360) >> 16); //returner vinklen
}

void loop() { //printer vinkel til skærm. Set u foran int32_t... for at få 360 i stedet for 180 og -180
...
void turnSensorSetup()
{
  ...
  // do some math and pointer magic to turn angle in seconds to angle in degrees
    lcd.print((((int32_t)turnAngle >> 16) * 360) >> 16); //beregner milisekunder til grader
    lcd.print(F("   "));
  }
  lcd.clear();
}
